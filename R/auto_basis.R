#' Automated spatial OLS regression with optimal basis and cluster selection
#'
#' @description
#' `auto_basis()` runs the full three-step spatial OLS pipeline automatically:
#'
...
#'
#' 2. **Optimal clusters** — runs [placebo()] using the first right-hand-side
#'    variable as the treatment and automatically selects the cluster count
#'    whose placebo rejection rate at the 5 % level is closest to 0.05.
#'
#' 3. **OLS regression** — calls [basis_regression()] with the chosen
#'    parameters and returns its output together with the selected values and
#'    the full placebo diagnostics.
#'
#' @param fm The regression formula in the form `y ~ treatment + controls`.
#'   The variable of interest must be the first one on the right-hand side.
#'   See [basis_regression()] for details.
...
#'   [basis_regression()] and `weights` in `df` for [placebo()].
...
#' \describe{
#'   \item{`result`}{A `fixest` object from [basis_regression()]. Can be
...
#'
#' # Regression table
#' modelsummary::modelsummary(list(`OLS-BCH` = res$result),
...
auto_basis <- function(fm, df, max_splines = 8, nSim = 1000,
  ...
  ols_names <- set_names(fm)
  var_dep   <- ols_names$gg[1]   # outcome
  var_expl  <- ols_names$orig_name  # treatment (first RHS variable)
